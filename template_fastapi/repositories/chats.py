...
from template_fastapi.models.chat import ChatHistory, ChatMessage, ChatRoom, ChatUser
...
class ChatRepository:
    ...
    def create_room(self, room_id: str, name: str, description: str | None = None) -> ChatRoom:
        """新しいチャットルームを作成"""
        if room_id in self.rooms:
            raise ValueError(f"Room {room_id} already exists")

        room = ChatRoom(room_id=room_id, name=name, description=description)
    ...
    def join_room(self, user_id: str, username: str, room_id: str, websocket: WebSocket) -> ChatUser:
        ...
        # ユーザー情報を作成/更新
        user = ChatUser(user_id=user_id, username=username, room_id=room_id)
    ...
    def add_message(self, user_id: str, message: str, room_id: str) -> ChatMessage:
        ...
        chat_message = ChatMessage(
            id=str(uuid.uuid4()), user_id=user_id, username=user.username, message=message, room_id=room_id
        )
    ...
    def get_messages(self, room_id: str, limit: int | None = None) -> list[ChatMessage]:
        """ルームのメッセージを取得"""
    ...
    def get_chat_history(self, room_id: str, limit: int | None = None) -> ChatHistory:
        """チャット履歴を取得"""
        messages = self.get_messages(room_id, limit)
        return ChatHistory(room_id=room_id, messages=messages, total_messages=len(messages))
    ...
    def get_user(self, user_id: str) -> ChatUser | None:
        """ユーザー情報を取得"""
    ...
    def cleanup_old_messages(self):
        ...
        for room_id in self.messages:
            self.messages[room_id] = [msg for msg in self.messages[room_id] if msg.timestamp > cutoff_time]
