...
def _load_chat_html() -> str:
    """チャットHTMLテンプレートを読み込む"""
    try:
        with open("/home/runner/work/template-fastapi/template-fastapi/template_fastapi/templates/chat.html", "r", encoding="utf-8") as f:
            return f.read()
...
@router.get("/chats/rooms/{room_id}/history", response_model=ChatHistory, tags=["chats"], operation_id="get_chat_history")
async def get_chat_history(
    room_id: str,
    limit: Optional[int] = Query(None, description="取得するメッセージ数の上限")
):
    """
    チャットルームの履歴を取得
    """
    ...
    try:
        ...
        if not username or len(username) > chat_settings.chat_max_username_length:
            await websocket.send_text(json.dumps({
                "type": "error",
                "message": f"ユーザー名は1文字以上{chat_settings.chat_max_username_length}文字以内で入力してください"
            }))
            ...
            try:
                ...
                if message_data.get("type") == "message":
                    ...
                    if len(message_text) > chat_settings.chat_max_message_length:
                        await websocket.send_text(json.dumps({
                            "type": "error",
                            "message": f"メッセージは{chat_settings.chat_max_message_length}文字以内で入力してください"
                        }))
                        continue
